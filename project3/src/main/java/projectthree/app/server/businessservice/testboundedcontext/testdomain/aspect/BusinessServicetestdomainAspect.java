package projectthree.app.server.businessservice.testboundedcontext.testdomain.aspect;
import projectthree.app.server.businessservice.aspect.BusinessAspect;
...
public class BusinessServicetestdomainAspect extends BusinessAspect {
    ...
    @Pointcut("execution(* projectthree.app.server.businessservice.testboundedcontext.testdomain..*(..))")
    ...
    public Object aroundfindAll(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        Object object = null;
        try {
            Log.out.println("GHYGG217100100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), joinPoint.getSignature().getDeclaringTypeName() + ":" + joinPoint.getSignature().getName());
            ...
            Log.out.println("GHYGG227100100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName());
        } catch (Exception e) {
            methodCallDetails.setPostCallDetails(HealthConstants.METHOD_EXCEPTION, HealthConstants.DEFAULT_EXCEPTION_ID);
            AppAlarm appAlarm = Log.getExceptionAlarm("GHY", "GG", e.getClass().getName());
