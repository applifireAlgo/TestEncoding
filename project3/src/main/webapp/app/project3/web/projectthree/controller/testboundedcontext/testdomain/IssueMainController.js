Ext.define('Project3.project3.web.projectthree.controller.testboundedcontext.testdomain.IssueMainController', {
     extend: 'Project3.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.IssueMainController',
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#IssueForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#IssueGrid');
               var tree = this.view.down('#IssueTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#IssueForm');
          var grid = this.view.down('#IssueGrid');
          var tree = this.view.down('#IssueTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#IssueGrid');
               ...
               var formPanel = this.getView().up().down('#IssueForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#IssueTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#IssueForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridConfig: function(data) {
          ...
          var gridStore = Ext.create('Ext.data.Store', {
               model: 'Project3.project3.shared.projectthree.model.testboundedcontext.testdomain.IssueModel',
               autoLoad: true,
